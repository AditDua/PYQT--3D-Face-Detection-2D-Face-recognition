...
class FaceID:
    ...
    def recognize_faces(self, img):
        ...
        for i in range(embs.shape[0]):
            pred, dist, conf = self.recognizer.identify(np.expand_dims(embs[i], axis=0), self.gal_embs, thresh=0.5)
            ids.append(
                [
...
class FaceRecognition:
    ...
    def identify(emb_src, embs_gal, thresh=None):
        # Calculate distances between source embedding and all gallery embeddings
        dists = cosine_distances(emb_src.reshape(1, -1), embs_gal)[0]
    
        # Get index of the closest match
        pred = np.argmin(dists)
    
        # Initialize confidence and distance
        conf = 0
        dist = dists[pred]
    
        # If a threshold is provided, use it to decide if prediction is valid
        if thresh is not None:
            if dists[pred] > thresh:
                idx = np.argsort(dists)  # Sort distances to get sorted indices
                if len(idx) > 1:
                # Calculate confidence if there are at least two distances
                    conf = (dists[idx[0]] - thresh) / (2.0 - thresh)  # Adjusted divisor to 2.0
                dist = dists[idx[0]]
                pred = None
            else:
                idx = np.argsort(dists)  # Sort distances to get sorted indices
                if len(idx) > 1:
                # Calculate confidence if there are at least two distances
                    conf = (dists[idx[1]] - dists[pred]) / 2.0  # Adjusted divisor to 2.0
                dist = dists[pred]
        else:
            idx = np.argsort(dists)  # Sort distances to get sorted indices 
            if len(idx) > 1:
